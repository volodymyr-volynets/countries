...
class Countries extends Import
{
    public $data = [
        'regions' => [
            'options' => [
    ...
    public function overrides()
    {
        ...
        $countries = [];
        $regions = [];
        $data = Imports::read(__DIR__ . '/countries.csv');
        unset($data['data']['Main Sheet'][0]);
        foreach ($data['data']['Main Sheet'] as $k => $v) {
            // if we need to import only specific countries
            if (!empty($this->options['cm_country_code']) && $this->options['cm_country_code'] != $v[1]) {
                continue;
            }
            $countries[$v[1]] = [
                'cm_country_code' => $v[1],
        ...
        // step 2: provinces
        $provinces = [];
        $data = Imports::read(__DIR__ . '/provinces.csv');
        unset($data['data']['Main Sheet'][0]);
        foreach ($data['data']['Main Sheet'] as $k => $v) {
            // if we need to import only specific countries
            if (!empty($this->options['cm_country_code']) && $this->options['cm_country_code'] != $v[2]) {
                continue;
            }
            $provinces[] = [
